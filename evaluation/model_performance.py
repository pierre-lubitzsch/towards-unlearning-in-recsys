...
def get_repeat_eval(pred_folder, dataset, size, fold_list, file, temporal_split, seed_list):
    ...
    for ind in fold_list:
        for seed in seed_list:
            ...
            if temporal_split:
                pred_file = f'{pred_folder}/{dataset}_pred{ind}_100_epochs_seed_{seed}.json'
            else:
...
def csv_int_list(s: str):
    return [int(item) for item in s.split(',') if item]


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--pred_folder', type=str, required=True, help='x')
    parser.add_argument('--fold_list', type=list, default=["0"], help='x')
    parser.add_argument("--seed_list", type=csv_int_list, default=None, help="seeds with which the models were trained")
    ...
    csv_seeds = ",".join(map(str, seed_list))
    eval_file = f'eval_results_dnntsp_100_seeds_{csv_seeds}.txt'
    f = open(eval_file, 'w')
    for dataset in ["tafeng"]:#['dunnhumby', 'tafeng', 'instacart']:
        f.write('############'+dataset+'########### \n')
        get_repeat_eval(pred_folder, dataset, 10, fold_list, f, temporal_split, seed_list)
        get_repeat_eval(pred_folder, dataset, 20, fold_list, f, temporal_split, seed_list)
